...
> [!IMPORTANT]
> **Основа проекта — роутинги для обхода блокировок в РФ.**  
> Для повседневного использования на смартфонах и ПК в России используйте основной профиль **DEFAULT** (или **JSONSUB**).

### 🌟 Основные профили (Обход блокировок в РФ)

*Предназначены для пользователей, находящихся в России/Беларуси. Локальные сервисы идут напрямую на максимальной скорости, а заблокированные зарубежные ресурсы (YouTube, Discord, Instagram, AI-сервисы) направляются в прокси-туннель.*

| Профиль | Назначение | Happ (iOS/Android/ПК) | INCY (iOS/Android/ПК) |
| :--- | :--- | :--- | :--- |
| **коридор роутинг**<br>*(DEFAULT)* | ⭐️ **Основной / Рекомендуемый.** Умное раздельное туннелирование: банки, маркетплейсы, Госуслуги и игры — напрямую; YouTube, Discord (вкл. голосовые), AI и заблокированное — через VPN. | [⚡ В 1 клик](HAPP/DEFAULT.DEEPLINK)<br>[📄 JSON URL](HAPP/DEFAULT.JSON) | [☁️ Autorouting](INCY/DEFAULT.AUTOROUTING)<br>[⚡ В 1 клик](INCY/DEFAULT.DEEPLINK) |
...
> [!WARNING]
> **Внимание! Эти профили являются узкоспециализированными и НЕ подходят для обычного повседневного использования в РФ.**  
> Не включайте их без прямой необходимости.

| Профиль | Назначение | Happ | INCY | Документация |
| :--- | :--- | :--- | :--- | :--- |
| **коридор роутинг (БС)**<br>*(WHITELIST)* | 🛡 **Режим учений ТСПУ / шатдауна.** Профиль экстремальной доступности. Пропускает напрямую **исключительно** критическую инфраструктуру РФ (белые списки: Госуслуги, Банки, СБП, связь, маркетплейсы, РЖД). Используется только при изоляции сети. | [⚡ В 1 клик](HAPP/WHITELIST.DEEPLINK)<br>[📄 JSON](HAPP/WHITELIST.JSON) | [☁️ Autorouting](INCY/WHITELIST.AUTOROUTING)<br>[⚡ В 1 клик](INCY/WHITELIST.DEEPLINK) | [WHITELIST](INCY/WHITELIST.JSON) |
| **коридор роутинг (релокант)**<br>*(RELOCANT)* | 🌍 **Обратный роутинг (Reverse Split).** Для тех, кто живёт **ЗА пределами РФ** (Сербия, Грузия, ЕС, Турция, СНГ). Весь мировой интернет идёт напрямую на скорости 1 Гбит/с, и **только** сервисы РФ (Госуслуги, Банки РФ, Кинопоиск, Faceit RU) идут через российский прокси-сервер. | [⚡ В 1 клик](RELOCANT/HAPP.DEEPLINK)<br>[📄 JSON](RELOCANT/HAPP.JSON) | [☁️ Autorouting](RELOCANT/INCY.AUTOROUTING)<br>[⚡ В 1 клик](RELOCANT/INCY.DEEPLINK) | [📖 Инструкция для релокантов и роутеров](RELOCANT/README.md) |
...
* **DEFAULT (Основной):** Предназначен для обхода блокировок в РФ. Локальные сервисы и диагностические чекеры идут напрямую, внешние платформы (YouTube, Discord) и AI-инструменты — через прокси-узел.
...
* **WHITELIST (БС, Специфический):** Режим максимальной защиты при учениях ТСПУ или отключении интернета. Доступны **только** белые списки РФ напрямую.
